#!/usr/bin/env python3
"""
Final script to extract TRREB Areas summary pages from Market Watch PDFs.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("final_trreb_areas_extract.log"),
...
def determine_property_type_from_text(text):
    ...
    if "SEMI-DETACHED HOUSES" in text.upper():
        ...
        return "TOWNHOUSE"
    elif "CONDO APT" in text.upper() or "CONDOMINIUM APARTMENT" in text.upper():
        return "CONDO APT"
    elif "ALL HOME TYPES" in text.upper():
    ...
    property_patterns = [
        ...
        (r'\bDETACHED CONDOMINIUM\b', "DETACHED CONDOMINIUM"),  # Check this before regular DETACHED
        ...
        (r'\bDETACHED\b(?!.*SEMI)(?!.*CONDO)', "DETACHED"),  # DETACHED but not SEMI-DETACHED or DETACHED CONDOMINIUM
        (r'\bSEMI-DETACHED HOUSES\b', "SEMI-DETACHED"),  # Explicit SEMI-DETACHED HOUSES
        (r'\bSEMI-DETACHED\b', "SEMI-DETACHED"),
        (r'\bCONDO APT\b', "CONDO APT"),
...
def is_trreb_areas_page(text, page_num, filename):
    ...
    # Check for ALL TREB/TRREB AREAS indicators
    has_trreb_indicator = any(x in text for x in [
        "ALL TRREB AREAS", 
        "ALL TREB AREAS", 
        "All TREB Areas", 
        "All TRREB Areas"
    ...
    # Specific check to exclude municipal breakdown pages
    if "City of Toronto Municipal Breakdown" in text or "Municipal Breakdown" in text:
        logging.info(f"  Skipping municipal breakdown page {page_num+1} in {filename}")
        return False
    
    # For TRREB Areas with data tables, include it even if it has some municipal data
    if has_trreb_indicator:
        return True
...
def determine_page_properties(text, date, page_num, filename):
    ...
    """
    # Check if it's a valid TRREB Areas page
    if not is_trreb_areas_page(text, page_num, filename):
        return None, False, None, None
    
    # Determine if it's year-to-date
    is_year_to_date = "YEAR-TO-DATE" in text
    ...
    # Create filename with numeric month format
    if is_year_to_date:
...
def extract_and_save_summary_pages(pdf_dir='pdfs', output_base_dir='split-pdfs', include_ytd=False, debug=False):
    """
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for i, page in enumerate(pdf.pages):
                    try:
                        ...
                        # Track property types found for this month
                        ...
                        if is_year_to_date and not include_ytd:
                            logging.info(f"  Skipping YEAR-TO-DATE page {i+1}: {page_title}")
                            skipped_ytd += 1
                            continue
                        ...
                        saved_pages += 1
                        logging.info(f"  Saved page {i+1}/{page_count}: {page_title} to {output_path}")
                        
                    except Exception as e:
                        logging.error(f"Error processing page {i+1} in {pdf_path}: {e}")
    ...
    missing_analysis = []
    expected_types = ['ALL HOME TYPES', 'DETACHED', 'SEMI-DETACHED', 'CONDO APT', 'TOWNHOUSE', 'CONDO TOWNHOUSE']
    ...
    # Print summary
    logging.info(f"Completed processing {total_pdfs} PDFs with {total_pages} total pages")
    logging.info(f"Saved {saved_pages} summary pages to {output_base_dir}")
    if skipped_ytd > 0:
        logging.info(f"Skipped {skipped_ytd} YEAR-TO-DATE pages (include_ytd={include_ytd})")
    if skipped_municipal > 0:
        logging.info(f"Skipped {skipped_municipal} municipal breakdown pages")
    
    print(f"\nProperty Types Found ({len(all_property_types)}):")
    ...
    if missing_analysis:
        for item in missing_analysis:
            print(f"  {item['date']} ({item['file']}): Missing {', '.join(item['missing'])}")
    else:
...
if __name__ == "__main__":
    ...
    INCLUDE_YTD_PAGES = False
    
    total_pdfs, total_pages, saved_pages, skipped_ytd, property_types = extract_and_save_summary_pages(
        include_ytd=INCLUDE_YTD_PAGES,
        debug=debug_mode
    )
    
    print(f"\nSummary:")
    print(f"Processed {total_pdfs} PDF files with {total_pages} total pages")
    print(f"Extracted and saved {saved_pages} individual 'SUMMARY OF EXISTING HOME TRANSACTIONS' pages")
    print(f"Skipped {skipped_ytd} YEAR-TO-DATE pages")
    print(f"Found {len(property_types)} distinct property types")
    print(f"Files are saved in split-pdfs/<year>/ with numeric month format (e.g., DETACHED_2020_07.pdf)")
