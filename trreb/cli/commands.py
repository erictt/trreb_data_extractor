...
def download():
    ...
    parser.add_argument(
        "--start-year", type=int, help="First year to download (default: 2016)"
    )
    parser.add_argument(
        "--log-level",
...
def extract_pages():
    ...
    parser = argparse.ArgumentParser(
        description="Extract specific pages from TRREB PDFs."
    )
    parser.add_argument(
        "--pdf", help="Process a specific PDF file (default: process all PDFs)"
    )
    parser.add_argument(
        "--log-level",
        choices=["DEBUG", "INFO", "WARNING", "ERROR"],
...
def process():
    ...
    parser = argparse.ArgumentParser(
        description="Process extracted pages into CSV format."
    )
    parser.add_argument(
        "--type",
...
def run_pipeline():
    ...
    parser = argparse.ArgumentParser(
        description="Run the full TRREB data extraction pipeline."
    )
    parser.add_argument(
        "--skip-download", action="store_true", help="Skip downloading PDFs"
    )
    parser.add_argument(
        "--skip-extract", action="store_true", help="Skip extracting pages"
    )
    parser.add_argument(
        "--skip-process", action="store_true", help="Skip processing CSVs"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate data after processing"
    )
