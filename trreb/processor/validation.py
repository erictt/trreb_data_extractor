...
from trreb.config import ALL_REGIONS, REGION_HIERARCHY
...
def validate_regions(df: pd.DataFrame, region_col: str = None) -> ValidationResult:
    ...
    min_expected_regions = 15  # A very minimal TRREB report would have at least 15 regions
...
def validate_numeric_columns(df: pd.DataFrame, expected_columns: Optional[List[str]] = None) -> ValidationResult:
    """
    ...
    if expected_columns is None:
        expected_columns = [
            ...
            "New Listings", "Active Listings", "Months Inventory",
...
def validate_data_consistency(df: pd.DataFrame) -> ValidationResult:
    ...
    if pd.api.types.is_numeric_dtype(df["Sales"]) and pd.api.types.is_numeric_dtype(df["Dollar Volume"]) and pd.api.types.is_numeric_dtype(df["Average Price"]):
    ...
    if "SNLR Trend" in df.columns and "Sales" in df.columns and "New Listings" in df.columns:
        if (
            pd.api.types.is_numeric_dtype(df["SNLR Trend"]) and
            pd.api.types.is_numeric_dtype(df["Sales"]) and
            pd.api.types.is_numeric_dtype(df["New Listings"])
        ):
...
def validate_time_series_continuity(df: pd.DataFrame, date_col: str, region_col: str = None) -> ValidationResult:
    """
...
def generate_validation_report(df: pd.DataFrame, date_col: Optional[str] = None) -> ValidationResult:
    """
